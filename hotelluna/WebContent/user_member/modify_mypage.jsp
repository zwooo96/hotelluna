...
// 		location.href="process_modify_mypage.jsp";
		$("#modifyFrm").submit();
   ...
   <div id="container" >
		...
		<div id="content">
    	 ...
    	 <img src="../common/images/member/mypage.png">
			...
			<form id="modifyFrm" method="post" action="process_modify_mypage.jsp">
			    ...
			    $("#M").attr("disabled", true);
					...
					<select class="form-control type" name="phoneType" id="phoneType">
						<option value="mobile"<%= "모바일".equals(pageContext.getAttribute("phoneT"))?" selected='selected'":""   %>>모바일</option>
						<option value="home"<%= "자택".equals(pageContext.getAttribute("phoneT"))?" selected='selected'":""   %>>자택</option>
					</select>
					</td>		
					<td>
					<select class="form-control country" id="phoneCountry" name="phoneCountry">
						<option value="+82"<%= "+82".equals(pageContext.getAttribute("phoneC"))?" selected='selected'":""   %>>대한민국(82)</option>
						<option value="+86"<%= "+86".equals(pageContext.getAttribute("phoneC"))?" selected='selected'":""   %>>중국(86)</option>
						<option value="+966"<%= "+966".equals(pageContext.getAttribute("phoneC"))?" selected='selected'":""   %>>사우디(966)</option>
					</select>
					</td>		
					<td>
						<input type="text" class="form-control num" id="phoneNum" name="phoneNum"  value="${phoneN }">
					</td>		
					</tr>
					</table>
					</div>
			      		<div class="btnLoc">
			     			<button type="button" class="btn btn-dark confirmBtn"  id="confirmBtn">완료하기</button>
			     		 </div>
			      </div>
			</form>  
    	</div>
